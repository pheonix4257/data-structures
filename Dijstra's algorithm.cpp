/* CODE BY B.G AKSHAYA
           148W1A0504
           CSE-1
Given an undirected/ directed graph G, find the shortest distance from source to all the vertices in the graph.  It can be assumed  that weights of all the edges are positive. 

Input: Number of vertices in G, Source vertex and Adjacency matrix A of graph G, with the value at A[i,j] is the weight of the edge joining vertex i and vertex j. 

Output: 
vertex number:distance from source 
